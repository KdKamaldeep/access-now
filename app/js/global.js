...
/*-------------------------------------------------------------------------------------------------------------------------------*/
/* Template Name: Access Now*/
/* Version: 1.0 Initial Release*/
/* Build Date: 25-02-2015*/
/* Author: Unbranded*/
/* Website: http://moonart.net.ua/Access Now/ 
/* Copyright: (C) 2015 */
/*-------------------------------------------------------------------------------------------------------------------------------*/
...
$(function () {
    ...
    /* 01 - VARIABLES */
    /*================*/
    var winW, winH, winScroll, _isresponsive, xsPoint = 767, smPoint = 991, mdPoint = 1199;
    ...
    function resizeCall() {
        ...
        $('.swiper-container.initialized').each(function () {
            var currentSwiper = swipers['swiper-' + $(this).attr('id')];
   ...
   $(document).on('click', '.open-icon, .close-icon', function () {
        $('header').toggleClass('active');
    });

    //accordeon
   $(document).on('click', '.accordeon-entry .title', function () {
        $(this).parent().toggleClass('active');
       $(this).next().slideToggle(300);
  });

    //testimonials
    $(document).on('click', '.testimonials-icons .entry div', function () {
    ...
    $(document).on('hover', '.circle-entry', function () {
    ...
    //main banner tabs
    $(document).on('click', '.tab-entry', function () {
        if ($(this).hasClass('active')) return false;
    ...
    $(document).on('click', '.prefix .selected-text', function () {
        $(this).parent().find('.prefix-drop-down').toggle();
    });

    $(document).on('click', '.prefix-drop-down div', function () {
    ...
    $(document).on('mouseleave', '.prefix', function () {
        $('.prefix-drop-down').hide();
    });

    //tabs switch
    $(document).on('click', '.tabs-desktop div', function () {
        var $t = $(this);
    ...
    $(document).on('change', '.tabs-switch-box select', function () {
    ...
    $(document).on('click', '.faq-switcher .side-menu-item', function () {
        if ($(this).hasClass('active')) return false;
    ...
    $(document).on('click', '.video-player .close-iframe', function () {
    ...
    $(document).on('click', '.theme-config .open', function () {
        $('.theme-config').toggleClass('active');
    });

    $(document).on('click', '.theme-config .colours-wrapper .entry', function () {
